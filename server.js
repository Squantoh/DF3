...
app.post("/api/fights/:code/vote-winner", authMiddleware, async (req,res)=>{
  ...
  if(pc && ac){
    ...
    await query(`INSERT INTO match_history(code, team_size, format, created_at, accepted_at, concluded_at, location, poster_ids, accepter_ids, poster_team_name, accepter_team_name, final_status, rating_delta)
      ...
      [f.code, f.team_size, f.format, f.created_at, f.accepted_at, f.location, f.poster_ids, f.accepter_ids, f.poster_team_name, f.accepter_team_name, winner==="POSTER"?"VICTORY":"DEFEAT", delta]
    );
